...
class ExerciseRepository {
  ...
  Future<List<LoadedExerciseModel>> getAllExercisesByWorkoutId(
      int workoutId) async {
  ...
  Future<List<MuscleGroupType>> getSecondaryMuscleGroups(int movementId) async {
    ...
    final List<Map<String, dynamic>> secondaryMuscleGroups =
        await db.rawQuery(dbQuery);

    return secondaryMuscleGroups
        .map((muscleGroup) =>
            MuscleGroupType.values.byName(muscleGroup['muscle_name']))
        .toList();
  }
}
